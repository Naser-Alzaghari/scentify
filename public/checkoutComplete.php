...
if (!empty($orderDetails)) {
    $latestOrderId = $orderDetails[0]['order_id'];
    $query_all_items = "
    SELECT 
        oi.order_item_id,
        oi.product_id,
    ...
    $stmt_items = $conn->prepare($query_all_items);
    $stmt_items->execute(['order_id' => $latestOrderId]);
    $itemsDetails = $stmt_items->fetchAll(PDO::FETCH_ASSOC);
} else {
    $itemsDetails = [];
}
?>
                                ...
                                <?php foreach ($itemsDetails as $item) : ?>
                                    <div class="card shadow-0 border mb-4">
                                        <div class="card-body">
                                            <div class="row">
                                                <div class="col-md-2">
                                                    <img src="./assets/img/gallery/<?php echo htmlspecialchars($item['product_image']); ?>"
